...
def _validate_path(path: str) -> str:
    ...
    try:
        common_prefix = os.path.commonpath([abs_path, _sandbox_root])
    except ValueError as exc:
        raise PermissionError(f"Access denied: {path} is outside sandbox") from exc

    if common_prefix != _sandbox_root:
